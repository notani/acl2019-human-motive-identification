...
def run(args):
    ...
    path_input = {'train': args.path_train,
                  'valid': args.path_valid,
                  'test': args.path_test}
    dataset = Dataset(path_input, verbose=verbose, logger=logger)

    data = dataset.ngram_encoder(args.ngram, tfidf=args.tfidf,
                                 return_df=False, return_idx=True)
    ...
    for params in expand_param_grid(param_grid):
        ...
        for c in range(NUM_MOTIVES):
            ...
            if 'sample_weight' in params:
                sample_weight = np.ones(y_.shape[0]) * params.pop('sample_weight')
            ...
            for split in ['train', 'valid']:
                for tag, X_ in X[split].items():
                    f1_[split][tag].append(f1_score(y_true=y[split][tag][:, c],
                                                    y_pred=clf.predict(X_)))
        if len(f1_['train']) == 0:
            continue
        for split in ['train', 'valid']:  # MEMO
            for tag, scores in f1_[split].items():
                f1[split][tag].append((params, np.mean(scores), np.std(scores)))

    best = sorted(zip(f1['valid'][target], f1['train'][target]),
    ...
    # Evaluation
    f1_, prec_, rec_ = [{'tv': defaultdict(list), 'test': defaultdict(list)}
                        for _ in range(3)]
    for c in range(NUM_MOTIVES):
        ...
        if 'sample_weight' in best_params:
            sample_weight = np.ones(y_.shape[0]) * best_params.pop('sample_weight')
        ...
        for split in ['tv', 'test']:
            for tag, X_ in X[split].items():
                ...
                prec_[split][tag].append(precision_score(y_true=y_, y_pred=pred))
    ...
    prec, rec, f1 = {}, {}, {}
    for split in ['tv', 'test']:
        prec[split] = {tag: (np.mean(values), np.std(values))
                       for tag, values in prec_[split].items()}
        rec[split] = {tag: (np.mean(values), np.std(values))
                      for tag, values in rec_[split].items()}
        f1[split] = {tag: (np.mean(values), np.std(values))
                     for tag, values in f1_[split].items()}

    print('||P|R|F1|')
    for tag in f1['test'].keys():
        buff = [tag]
        buff += ['{:.3f} ({:.3f})'.format(*val)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', dest='path_train', required=True,
                        help='path to training file')
    parser.add_argument('--valid', dest='path_valid', required=True,
                        help='path to validation file')
    parser.add_argument('--test', dest='path_test', required=True,
                        help='path to testing file')
    parser.add_argument('--ngram', nargs=2, type=int, default=(1, 3),
                        help='range of ngram')
    parser.add_argument('--no-class-weight', dest='class_weight',
                        action='store_false',
                        help='ignore empirical class weight')
    parser.add_argument('--tfidf', action='store_true',
                        help='use TFIDF transformer')
