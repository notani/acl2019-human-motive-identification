...
MOTIVES_7 = [
    'Self-fulfill', 'Appreciating Beauty',
    'Social Relating', 'Family', 'Health',
...
def make_vocab(texts, ngram=(1, 3), min_freq=2, pad=False, unk=False):
    ...
    vocab = [w for w, v in reversed(sorted(counter.items(), key=lambda t: t[1]))
             if v >= min_freq]
    indexer = {}
    if pad:
...
class Dataset:
    def __init__(self, path_input,
                 verbose=False, logger=init_logger('Dataset')):
        ...
        for split, filename in path_input.items():
            if isinstance(filename, str):
                self.df[split]['_'] = pd.read_table(filename)
            elif isinstance(filename, dict):
                for tag, filename_ in filename.items():
                    self.df[split][tag] = pd.read_table(filename_)
            if self.verbose:
                self.logger.info('Read {} items from {}'.format(len(self.df[split]), filename))


    def ngram_encoder(self, ngram, tfidf=False, df_unlabeled=None,
                      return_df=False, return_idx=True):
        """Iterate data"""
        indices = {}
        # Feature extraction
        df_train = pd.concat(self.df['train'].values(), ignore_index=True, sort=False)
        indexer, counter = make_vocab(df_train['text_tkn'].values, ngram=ngram)
        X, y = {}, {}
        for split in ['train', 'valid']:
            X[split] = {tag: text2ids(df_['text_tkn'].values, indexer, ngram=ngram)
                        for tag, df_ in self.df[split].items()}
            y[split] = {tag: df_[MOTIVES].values.astype(int)
                        for tag, df_ in self.df[split].items()}
        ...
        if df_unlabeled is not None:
            X_unlabeled = text2ids(df_unlabeled['text_tkn'].values,
                                   indexer, ngram=ngram)

        if tfidf:
            transformer = TfidfTransformer().fit(X['train']['_'])
            for split in ['train', 'valid']:
                X[split] = {tag: transformer.transform(X_)
                              for tag, X_ in X[split].items()}
            if df_unlabeled is not None:
                X_unlabeled = transformer.transform(X_unlabeled)

        # Feature extraction (train+valid)
        self.df['tv'] = {tag: pd.concat((self.df['train'][tag], self.df['valid'][tag]),
                                        ignore_index=True)
                         for tag in self.df['train'].keys()}
        df_tv = pd.concat(self.df['tv'].values(), ignore_index=True, sort=False)
        ...
        if df_unlabeled is not None:
            X_unlabeled_tv = text2ids(df_unlabeled['text_tkn'].values,
                                      indexer, ngram=ngram)
