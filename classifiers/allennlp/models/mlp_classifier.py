...
class MlpClassifier(Model):
    ...
    def forward(self,
        ...
        embeddings = torch.mul(embeddings, mask.unsqueeze(2).float())

        # SWEM

        length= embeddings.size()[1]
        masked_sum = mask.sum(dim=1).unsqueeze(1).float()
        swem_aver = embeddings.sum(dim=1) / masked_sum
        if self.aggregation_type == 'aver':
            encoder_out = swem_aver
        else:
            swem_max = F.max_pool1d(embeddings.transpose(1, 2), length).squeeze()
            encoder_out = torch.cat([swem_aver, swem_max], dim=1)
